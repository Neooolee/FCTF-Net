...
class My_loss(torch.nn.Module):
    ...
    def forward(self, x, y):
        temp = torch.mean(x-y)
        return torch.mean(torch.pow((x - y-temp), 2))
        
class My_log_loss(torch.nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self, x, y):
        temp = torch.abs(x-y)
        return torch.mean(temp - 0.1*torch.log(temp+0.1)+ 0.1*torch.log(torch.tensor([0.1]).cuda()))
